...
// facilitatorEvmSigner implements evmmech.FacilitatorEvmSigner using a local
// ECDSA key and a JSON-RPC endpoint. In production, wrap a KMS/HSM here and
// harden nonce management, gas estimation, and receipt polling.
...
func (s *facilitatorEvmSigner) VerifyTypedData(
	...
	typesIn map[string][]evmmech.TypedDataField,
	primaryType string,
	message map[string]interface{},
	signature []byte,
) (bool, error) {
	td := buildTypedData(domain, typesIn, primaryType, message)
...
func (s *facilitatorEvmSigner) ReadContract(
	...
	// Single-return methods (e.g. balanceOf, allowance) hand back the bare
	// value; multi-return methods (e.g. receivers → (totalClaimed, totalSettled))
	// must hand back the full []interface{} so callers can index every output.
	switch len(results) {
	case 0:
		return nil, nil
	case 1:
